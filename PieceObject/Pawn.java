...
public class Pawn extends Piece{
    ...
    public void possibleMoveWC(Piece[] b, int ind){
        ...
        if(ind >= bound + 8 * sign && ind <= bound + 15 * sign){
            if(b[ind + offset[1] * sign] == null) move.add(ind + offset[1] * sign);
            if(b[ind + offset[3] * sign] == null) move.add(ind + offset[3] * sign);
        } else {
            if(b[ind + offset[1] * sign] == null
            && ind + offset[1] * sign <= 63
            && ind + offset[1] * sign >= 0) move.add(ind + offset[1] * sign);
        }
    ...
    public void possibleCapture(Piece[] b, int ind){
        ...
        if(b[pos[0]] != null && b[pos[0]].getCol() != b[ind].getCol()) move.add(ind + offset[0] * sign);
        if(b[pos[1]] != null && b[pos[1]].getCol() != b[ind].getCol()) move.add(ind + offset[2] * sign);
